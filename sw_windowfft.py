...
def read_file_data(filepath):
    ...
    for raw_val in raw_data:
        ...
        time = 0
        try:
            float_amp_str = raw_val[0].replace(',', '.')
            amp = float(float_amp_str)
        except:
            amp = None
        finally:
            float_time_str = raw_val[1].replace(',', '.')
            time = float(float_time_str)
...
def plot_graph(data, out_filepath, lb_freq_start=0.01, lb_freq_end=4.0, lb_freq_num=100000, to_display=False, save_to_disk=True):
    ...
    # Calculate FFT
    spectrogram = np.abs(dct(y))
    spectrogram_freq = [freq for freq in range(len(y))]

    # Create figure with 2 subplots
    fig = plt.figure()
    source_ax = fig.add_subplot(211)
    spectrogram_ax = fig.add_subplot(212)

    #Now make a plot of the input data:
    source_ax.plot(x, y, 'b+')

    #Then plot the normalized periodogram:
    spectrogram_ax.plot(spectrogram_freq, spectrogram)
    ...
    # Generate output
    for idx, freq in enumerate(spectrogram_freq):
        if freq == 0:
            continue
        period = 2 * math.pi / freq
        output_data.append([freq, period, spectrogram[idx], time_value])
